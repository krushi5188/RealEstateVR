...
*
    *   **Circulation Path Analysis:**
        *   **Status:** 🟠 **Placeholder**
        *   **Evidence:** The server endpoint `/analyze-circulation` **ignores the user's model** and runs a real pathfinding algorithm on an internal, **hardcoded dummy floor plan**. The result displayed to the user is unrelated to their design.
    *   **Natural Light Analysis:**
        *   **Status:** 🟠 **Placeholder**
        *   **Evidence:** The `light-analyzer.js` module on the client contains **no real analysis logic**. It is a placeholder that returns a static, hardcoded score.
    *   **Ergonomics & Accessibility Audit:**
        *   **Status:** 🟠 **Placeholder**
        *   **Evidence:** The server endpoint `/audit-accessibility` **ignores the user's model** and returns a **hardcoded, static JSON report**. It performs no analysis at all.
